...
news-monitor 负责采集 RSS/Atom、RSSHub 和 X，保存原文，调用配置的模型（本机为 DeepSeek）生成中文标题和每条不超过 200 字的中文摘要（默认不翻译正文），再输出结构化新闻列表。调用方 Agent 负责精选、事件关联、变化判断和报告写作；可选的 HTML 渲染器只负责校验与排版。
...
主 YAML 支持 `includes` 分层加载与 `sourceFiles` 来源列表，兼容旧单文件配置。RSS 在 [config/sources/rss.yaml](config/sources/rss.yaml)，X 在 [config/sources/x.yaml](config/sources/x.yaml)，Blog 使用现有 [feeds/hn-popular-blogs.json](feeds/hn-popular-blogs.json)。[运行参数](config/runtime.yaml)、[精选规则](config/editorial.yaml)、[LLM 模板](config/llm.example.yaml)和[提示词](config/prompts.yaml)分别维护。本机专用配置放在 Git 忽略的 `config/local/`，密钥仍通过环境或 Pi 读取。

完整字段、覆盖顺序、路径语义和开关见 [配置说明](config/README.md)。`opencli.enabled: false` 可全局停用 X 采集，避免自动操作浏览器。配置改动后重新运行 CLI 即生效，无需重新构建；代码更新仍需构建。
...
`collect` / `news` 从不调用内置精选或专题分析模型，即便旧配置中 `curation.enabled: true`。`curation.interests` / `minPicks` / `maxPicks` / `selectionCriteria` 作为偏好交给外部 Agent。旧 `--analyze` 已从 CLI 移除。旧编辑模块和缓存保留供历史实验读取，不在新运行流程中使用。
...
筛选发生在快照层：RSS 返回的原始条目照常归档；报告刷新阶段只摘要窗口内的新闻和博客，历史博客不占当天摘要额度；独立 `collect` 的中文处理与归档预览仍保留。快照 `publicationFilter` 和 `blogPublicationFilter` 分别记录新闻、博客的纳入数量及排除原因；缺少或无效日期导致内容被排除时，非空快照标记为 partial。旧快照和已发布报告不会自动改写，重新生成后才应用新规则。
...
10:00/20:00 编辑任务由宿主 Agent 安排，调用上面的 `news --refresh` 并保存实际早报快照。只采集两次会降低高频来源的覆盖，RSS 已滚出的条目无法补回；报告只处理窗口内文章的中文摘要；需要处理历史博客时可手动运行 `collect --channel blogs`。程序不会自行安装定时任务。
...
博客与其他新闻统一生成中文标题和不超过 200 字的中文摘要，不翻译全文。独立 `collect` 的 `localization.blogBatchSize` 默认每轮新处理 20 篇（0 为只复用缓存），历史积压可继续处理；此限制不适用于报告。`news` / `report` 必须等待当前候选全部完成摘要或明确失败后，才冻结快照、生成 HTML。报告窗口内博客无论多少都需处理，不以 pending 提前放行。

`news --refresh` 先完成采集归档，再按发布时间筛选，在报告阶段统一执行摘要，每篇只走一轮配置的重试流程，避免采集与快照阶段重复尝试。成功复用缓存，超过模型重试次数的失败保留原文与失败状态；模型缺失或中文处理关闭会明确记为本次无法处理，不伪装为摘要成功。摘要进程尚未结束时不生成快照；`render` 拒绝含 pending/disabled 的旧快照，须重新导出。RSS 并发由 `collection.rssConcurrency` 控制，X 仍串行；同归档不要重复启动采集。

## 精选规则

[config/editorial.yaml](config/editorial.yaml) 默认 `minPicks: 10`、`maxPicks: 20`，由报告编辑模型按质量自行决定数量，每条至少满足以下一个条件：

1. 有切实影响并且影响力大、影响程度深、影响范围广。
2. 内容非常优质、非常值得阅读。
3. 属于某领域的重大进展。
4. 非常奇怪，超出常规认知。

每条 reason 要用来源证据说明符合哪一条件及具体事实；不能用热度或标题夸张代替重要性，也不能为猎奇放大未经证实的说法。同一事件最多精选一次，博客不参与。通常选择 10–20 条；若合格新闻确实不足 10 条，明确说明不足原因，不用不合格内容凑数。

## 中文摘要与状态

默认 `localization.mode: summary`：全部 RSS、X、Blog 只生成中文标题与不超过 200 个 Unicode 字符（含标点）的中文摘要，不生成正文译文。原文完整归档，HTML 支持展开原文；只总结实际提供的内容，不把简介当全文。常规文章一次请求，超长文按 `summaryChunkChars`（默认 24000）分段摘要，再合并所有段落，避免遗漏尾部；中文原题保持。只有显式 `mode: full` 才使用旧全文处理模式。

模型调用的超时、重试、输出 token、temperature 和提示词都支持配置。缓存按原文、模型和有效提示词/生成参数复用；默认历史缓存可直接提取已有标题和摘要，无需为迁移重跑模型，也不展示旧译文。独立采集缺少模型时保留 pending；报告流程将无法执行或重试耗尽记为明确 failed，保留原文，重试复用成功分段。新模式主要减少长文分段次数和输出 token；短文以前也是一次调用，不承诺所有文章的调用次数都下降。
...
- `chinese/` 保存中文摘要及成功分段缓存，旧译文缓存保留。
- `snapshots/<uuid>/news.json` 是 Agent 使用的紧凑列表；同目录 `reading-pack.json` 保存原文和中文摘要证据（旧全文模式可含译文），用 SHA-256 绑定。渲染验证原始版本与摘要一致。
