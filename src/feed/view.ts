...
export function renderFeed(items: ReadingItem[], results: SourceResult[], now: string, digest?: Digest, localization?: LocalizationStats, curation?: Curation, options: { email?: boolean } = {}): string {
  ...
  const tier = (item: ReadingItem) => ready ? curation.entries[item.id]?.tier || 'reading' : 'reading'
  const groups = { picks: items.filter(i => tier(i) === 'picks'), reading: items.filter(i => tier(i) === 'reading'), other: items.filter(i => tier(i) === 'other') }
  for (const list of Object.values(groups)) list.sort((a, b) => ((curation?.entries[a.id]?.rank || 99) - (curation?.entries[b.id]?.rank || 99)) || ((curation?.entries[b.id]?.score || 0) - (curation?.entries[a.id]?.score || 0)))
  const initial = groups.picks.length ? 'picks' : 'reading'
  const itemBody = (item: ReadingItem) => {
    const chinese = item.chinese
    const entry = curation?.entries[item.id]
  ...
  const labels = { picks: '精选', reading: ready ? '继续阅读' : '全部内容', other: '其他资讯' }
  const panels = (Object.keys(groups) as Array<keyof typeof groups>).map(key => `<section class="panel" id="panel-${key}" role="tabpanel" aria-labelledby="tab-${key}" data-panel="${key}">${groups[key].map((item, i) => article(item, i, key)).join('')}<p class="empty" data-empty ${groups[key].length ? 'hidden' : ''}>这个分组下没有匹配内容。可切换分类或清除搜索。</p></section>`).join('')
