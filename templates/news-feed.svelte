...
<div class="reader">
  ...
  {#if report}
    ...
    {#if report.summary.trim()}<p class="report-note">{report.summary}</p>{/if}
    {#if report.sections.length}
    <section class="overview" aria-labelledby="overview-heading">
      <h2 id="overview-heading" class="overview-heading">新闻综述</h2>
      <ol class="overview-list">
      {#each report.sections as section, sectionIndex}
        <li>
          {#if section.kind !== 'overview'}<h3><span>{kinds[section.kind] || ''}</span> {section.title}</h3>{/if}
          <p>{section.body}<sup class="citations">{#each section.sources as citation, citationIndex}<a href={safe(citation.url) || undefined} target="_blank" rel="noopener noreferrer" title={`${citation.before ? '早前：' : ''}${citation.source} · ${citation.title}`} aria-label={`参考来源 ${report.sections.slice(0, sectionIndex).reduce((n, s) => n + s.sources.length, 0) + citationIndex + 1}：${citation.before ? '早前：' : ''}${citation.source} · ${citation.title}`}>[{report.sections.slice(0, sectionIndex).reduce((n, s) => n + s.sources.length, 0) + citationIndex + 1}]</a>{/each}</sup></p>
        </li>
      {/each}
      </ol>
    </section>
...
<style>
  ...
  header{padding:28px 0 16px}.dateline{letter-spacing:.12em;font-size:11px;color:var(--muted);display:flex;justify-content:space-between;gap:10px}.dateline span{letter-spacing:0}h1{font-family:'Songti SC','Noto Serif CJK SC',serif;font-size:clamp(25px,4vw,35px);line-height:1.4;letter-spacing:.02em;margin:12px 0 0;font-weight:700}.recent{font-size:12px;color:var(--muted);margin:10px 0 0}.report-note{margin:0 0 20px;font-size:11px;line-height:1.8;color:var(--muted);white-space:pre-wrap}.overview{margin:0 0 26px;padding:20px 24px;background:var(--overview-bg,#f1f5f2);border-radius:8px}.overview-list{list-style:decimal;margin:0;padding-left:1.5em;font-size:13px}.overview-list li{padding-left:4px}.overview-list li+li{margin-top:12px}.overview-list li::marker{color:var(--muted);font-variant-numeric:tabular-nums}.overview p{margin:0;font-size:13px;white-space:pre-wrap;line-height:1.9}.overview .overview-heading{font-size:12px;font-weight:600;letter-spacing:.08em;color:var(--accent);margin:0 0 14px}.overview h3{font-size:14px;margin:12px 0 6px}.overview h3 span{font-size:11px;color:var(--accent);margin-right:5px}.citations{font-size:10px;line-height:0;vertical-align:super;color:var(--accent);white-space:nowrap}.citations a{margin-left:3px}
  ...
  @media(max-width:520px){.reader{font-size:14px}.toolbar{gap:8px}.tabs{gap:18px}.tabs button{font-size:13px}.dateline{flex-direction:column;gap:2px}.pick h2{font-size:17px}.overview{padding:16px;margin-bottom:20px}}
  @media(prefers-color-scheme:dark){.reader{--overview-bg:#24382d;--ink:#dbe5dd;--muted:#a1ada4;--line:#3a4840;--accent:#91c6a8}.chosen{background:#2d4337}.error{background:#372b24;color:#e4bca3;border-color:#655043}}
